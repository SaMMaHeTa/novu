...
describe('Unseen Count - GET /widget/notifications/unseen', function () {
  ...
  it('should return unseen count with a seen filter', async function () {
    ...
    await axios.post(
      `http://192.168.10.5:${process.env.PORT}/v1/widgets/messages/markAs`,
  ...
  async function getFeedCount(query = {}) {
    const response = await axios.get(`http://192.168.10.5:${process.env.PORT}/v1/widgets/notifications/count`, {
      params: {
        ...query,
      },
      headers: {
