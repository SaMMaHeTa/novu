...
describe('GET /widget/notifications/feed', function () {
  ...
  async function getSubscriberFeed(query = {}) {
    const response = await axios.get(`http://192.168.10.153:${process.env.PORT}/v1/widgets/notifications/feed`, {
      params: {
        page: 0,
        ...query,
      },
  ...
  async function markMessageAsSeen(messageId: string) {
    return await axios.post(
      `http://192.168.10.153:${process.env.PORT}/v1/widgets/messages/markAs`,
