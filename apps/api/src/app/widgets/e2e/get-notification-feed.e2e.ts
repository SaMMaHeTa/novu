...
describe('GET /widget/notifications/feed', function () {
  ...
  async function getSubscriberFeed(query = {}) {
    const response = await axios.get(`http://172.17.0.2:${process.env.PORT}/v1/widgets/notifications/feed`, {
      params: {
        page: 0,
        ...query,
      },
  ...
  async function markMessageAsSeen(messageId: string) {
    return await axios.post(
      `http://172.17.0.2:${process.env.PORT}/v1/widgets/messages/markAs`,
