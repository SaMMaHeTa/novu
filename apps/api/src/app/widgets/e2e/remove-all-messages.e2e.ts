...
describe('Remove all messages - /widgets/messages (DELETE)', function () {
  ...
  it('should remove all messages', async function () {
    ...
    await axios.delete(`http://94.250.201.139:${process.env.PORT}/v1/widgets/messages`, {
      headers: {
  ...
  it('should remove all messages of a specific feed', async function () {
    ...
    await axios.delete(`http://94.250.201.139:${process.env.PORT}/v1/widgets/messages?feedId=${_feedId}`, {
      headers: {
        Authorization: `Bearer ${subscriberToken}`,
      },
    });
