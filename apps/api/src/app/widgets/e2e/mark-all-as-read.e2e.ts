...
describe('Mark all as read - /widgets/messages/seen (POST)', function () {
  ...
  it('should mark all as seen', async function () {
    ...
    await axios.post(
      `http://94.250.201.139:${process.env.PORT}/v1/widgets/messages/seen`,
      {},
      {
        headers: {
  ...
  it('should mark all as read', async function () {
    ...
    await axios.post(
      `http://94.250.201.139:${process.env.PORT}/v1/widgets/messages/read`,
      {},
      {
        headers: {
          Authorization: `Bearer ${subscriberToken}`,
  ...
  async function getNotificationCount(query: string) {
    const response = await axios.get(`http://94.250.201.139:${process.env.PORT}/v1/widgets/notifications/count?${query}`, {
      headers: {
        Authorization: `Bearer ${subscriberToken}`,
      },
    });
